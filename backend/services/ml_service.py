...
def generate_ml_features(mongo, email):
    ...
    try:
        ...
        return pd.DataFrame([[total_carbon, avg_carbon]], columns=["total_weekly_carbon", "avg_daily_carbon"])
...
def predict_carbon_risk(mongo, email):
    ...
    if not os.path.exists(MODEL_PATH):
        return {"error": "Model file not found. Please run train_model.py first.", "risk_level": "Unknown"}
