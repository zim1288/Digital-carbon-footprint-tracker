...
from services.ml_service import generate_ml_features, generate_training_dataset, predict_carbon_risk
...
def fetch_from_gemini(prompt):
    ...
    if api_key:
        ...
        url = (
            "https://generativelanguage.googleapis.com/v1beta/models/" f"gemini-2.0-flash:generateContent?key={api_key}"
        )
        ...
        try:
            ...
            # If Google API fails (429 Quota limit, 503, etc.)
            print(f"⚠️ Gemini API Failed ({e}). " "Switching to Capstone Demo Fallback mode!")

    # 2. OFFLINE FALLBACK
    if "Analyze my daily digital" in prompt:
        return (
            "Your biggest offender today is Video Streaming. "
            "This is equivalent to boiling a kettle 3 times! To make a "
            "high-impact reduction, try dropping your video resolution "
            "to 720p or downloading videos on Wi-Fi."
        )
    else:
        return (
            "That is a great question! Lowering screen brightness, turning "
            "off auto-play on social media, and deleting heavy cloud files "
            "are fantastic ways to reduce your digital carbon footprint."
        )
...
def recommendations(email):
    try:
        ...
        if most_activity and most_activity.get("activity_type"):
            ...
            smart_msg = (
                f"Your highest carbon activity is {activity_name} "
                f"({carbon_value} g CO₂). Try reducing it to lower your footprint."
            )
