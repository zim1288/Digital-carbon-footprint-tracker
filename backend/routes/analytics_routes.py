...
def get_today_usage(email):
    try:
        ...
        return jsonify({
            "streaming": 0, "calls": 0, "social": 0, "general": 0
        }), 200
...
# ==============================
# NEW: Fetch Today's Complete Breakdown (Sliders + Manual) for Pie Chart
# ==============================
...
def get_weekly_history(email):
    try:
        ...
        # FIXED: Removed the "is_daily_summary: True" filter so it gets ALL data
        activities = list(mongo.db.activities.find({
            "user_email": email,
            "date": {"$gte": seven_days_ago}
        }))
        ...
        formatted_history = [
            {"day": day, "emissions": round(history_map[day], 2)} 
